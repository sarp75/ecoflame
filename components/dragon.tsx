    ...
    <section className="relative w-full max-w-3xl text-center">
      ...
      >
        <div className="absolute inset-x-6 top-4 flex justify-between text-xs uppercase text-white/70">
          <span>habitat · {HABITATS[stage].title}</span>
          <span>lvl {level}</span>
        </div>
        <div className="flex flex-col items-center justify-center gap-6 pt-8">
          <button
            ...
            className={`relative isolate flex h-48 w-48 items-center justify-center rounded-full bg-gradient-to-r from-lime-300/20 via-emerald-400/30 to-lime-200/20 text-6xl text-white shadow-[0_0_35px_rgba(124,255,170,0.4)] ring-2 ring-white/10 transition-all duration-300 ${dragonPose}`}
            aria-label="ejderhayı sev"
          >
            🐉
            <span className="absolute inset-0 -z-10 rounded-full bg-lime-400/20 blur-3xl" />
          </button>
          <div className="relative rounded-2xl bg-white/95 px-5 py-3 text-sm font-medium text-emerald-900 shadow-xl shadow-emerald-900/30" aria-live="polite">
            {activeTip}
          ...
          <div className="flex flex-wrap justify-center gap-3 text-xs uppercase tracking-[0.35em] text-white/80">
            <span className="rounded-full border border-white/40 bg-white/10 px-4 py-1">coins {me.coins}</span>
            <span className="rounded-full border border-white/40 bg-white/10 px-4 py-1">xp {me.total_xp}</span>
            <span className="rounded-full border border-white/40 bg-white/10 px-4 py-1">lvl {level}</span>
          </div>
          <div className="grid w-full grid-cols-2 gap-3 text-[10px] uppercase tracking-[0.35em] text-lime-100/80">
            <span className="rounded-2xl border border-lime-200/40 bg-lime-300/10 px-3 py-2">ruh akışı +{me.coins}</span>
            <span className="rounded-2xl border border-emerald-200/40 bg-emerald-300/10 px-3 py-2">denge {((level+7) * 7)-8}</span>
          </div>
        </div>
      </div>
    </section>
